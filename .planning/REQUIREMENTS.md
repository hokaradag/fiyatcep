...
- [ ] **QUAL-01**: Kullanıcı tüm arama ve görüntüleme ekranlarında Türkçe karakterleri doğru eşleştirebilir (tüm `_normalizeText` kopyaları `TextNormalizer` utility'sine taşınır)
- [ ] **QUAL-02**: Kullanıcı network hatası aldığında anlamlı bir hata mesajı görür — opaque Exception string değil (tüm provider'lardaki `throw Exception(message)` typed `AppException` ile değiştirilir)
...
- [ ] **QUAL-07**: `CarrefourSA` ismi uygulama genelinde tek bir yazımla kullanılır (tüm tutarsız `Carrefoursa`/`CarrefourSA` varyasyonları normalize edilir)
...
- [ ] **COMP-02**: Kullanıcı bir ürünün fiyat geçmişini zaman grafiğinde görebilir (1H / 1A / 3A / 1Y zaman seçici, dokunma ile exact fiyat tooltip'i)
